...
export const metadata: Metadata = {
  title: "At home · activity",
  description: "Full archive of essays, issues, field notes, and case files.",
};
...
export default async function AtHomeArchivePage() {
  const events = await getRecentWorkEvents({ limit: ARCHIVE_LIMIT });
  const now = Date.now();
      ...
      <section className="border-b border-rule">
        <div className="max-w-4xl mx-auto px-6 pt-16 md:pt-20 pb-12 text-center">
          ...
          <p className="deck max-w-xl mx-auto fade-up stagger-3">
            Everything published on the site, newest first. Essays,
            issues, field notes, case files.
          </p>
        </div>
      </section>
      ...
      <section className="max-w-3xl mx-auto px-6 py-14 md:py-20">
        ...
        {events.length === 0 ? (
          <p
            ...
            style={{ fontSize: "1.05rem" }}
          >
            Nothing on the shelf yet. The first essays and field notes
            will land here as they publish.
          </p>
        ) : (
